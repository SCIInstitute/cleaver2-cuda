...
SizingFieldOracle::SizingFieldOracle(AbstractScalarField *sizingField, const BoundingBox &bounds) :
...
void recurseCheck1(OTCell *cell, Octree *tree)
{
    ...
    for(int i=0; i < 8; i++)
    {
        float childLFS = pCell->children[i]->minLFS;
...
void recurseCheck2(OTCell *cell, Octree *tree, Cleaver::AbstractScalarField *field)
{
    ...
    int min_x = cellBounds.minCorner().x;   int max_x = cellBounds.maxCorner().x;
    int min_y = cellBounds.minCorner().y;   int max_y = cellBounds.maxCorner().y;
    int min_z = cellBounds.minCorner().z;   int max_z = cellBounds.maxCorner().z;
    ...
    for(int k=min_z; (k + 0.5) < max_z; k++)
    {
        for(int j=min_y; (j + 0.5) < max_y; j++)
        {
            ...
            {
                float LFS = field->valueAt(i+0.5, j+0.5, k+0.5);
...
void SizingFieldOracle::setSizingField(AbstractScalarField *sizingField)
{
...
//                   (bounds.center().z / volume->bounds().size.z)*sizingField.bounds().size.z);
    ...
    float voxel_scale = ((Cleaver::ScalarField<float>*)m_sizingField)->scale().x;
