...
void CleaverMesherImp::adaptCell(OTCell *cell)
{
    ...
    int max_x = domainBounds.maxCorner().x;
    int max_y = domainBounds.maxCorner().y;
    int max_z = domainBounds.maxCorner().z;
    ...
    */
    ...
    float LFS = m_sizingOracle->getMinLFS(cell->xLocCode, cell->yLocCode, cell->zLocCode, cell->level);
...
Octree* CleaverMesherImp::createOctreeBottomUp()
{
    ...
    for(int k=0; k < d; k++)
    {
        for(int j=0; j < h; j++)
        {
            for(int i=0; i < w; i++)
            {
                float lfs = m_sizingField->valueAt(i+0.5, j+0.5, k+0.5);
            ...
            for(int x_dir=-1; x_dir <= 1; x_dir++)
            {
                for(int y_dir=-1; y_dir <= 1; y_dir++)
                {
                    for(int z_dir=-1; z_dir <= 1; z_dir++)
                    {
                        ...
                        int nbr_x = cur_x + shift*x_dir;
                        int nbr_y = cur_y + shift*y_dir;
                        int nbr_z = cur_z + shift*z_dir;

                        // if outside, move on
                        if(nbr_x < 0 || nbr_y < 0 || nbr_z < 0)
                            continue;
                        else if(nbr_x >= maxCode || nbr_y >= maxCode || nbr_z >= maxCode)
                            continue;
...
void CleaverMesherImp::balanceOctreeNew()
{
    ...
    while(!q.empty())
    {
        ...
        bool has_children = cell->children[0];
        // if there are children, enqueue them
        if(has_children)
        {
            for(int i=0; i < 8; i++)
...
void CleaverMesherImp::computeAlphas(bool verbose)
{
    ...
    for(edge_iter = m_bgMesh->halfEdges.begin(); edge_iter != m_bgMesh->halfEdges.end(); edge_iter++)
    {
        ...
        half_edge->alpha = alpha_init;
        half_edge->alpha_length = half_edge->alpha*length(half_edge->vertex->pos() - half_edge->mate->vertex->pos());
    }
    ...
    */
    ...
    for(unsigned int v=0; v < m_bgMesh->verts.size(); v++)
    {
        ...
        for(int e=0; e < adjEdges.size(); e++)
        {
            ...
            float viol_dist = alpha_init * length(ray);
        ...
        for(int e=0; e < adjEdges.size(); e++)
        {
            ...
            float old_dist = alpha_init * length(ray);
...
void CleaverMesherImp::computeAlphasAlt(bool verbose)
{
    ...
    for(unsigned int t=0; t < m_bgMesh->tets.size(); t++)
    {
        ...
        for(unsigned int vidx=0; vidx < VERTS_PER_TET; vidx++)
        {
            ...
            vec3 alt = proj - v0;
            float req_length = alpha_init*length(alt);
            ...
            for(int v=0; v < 4; v++)
            {
                if(alpha_length[v] > req_length)
                {
                    ...
                    for(int e=0; e < adjEdges.size(); e++)
                    {
                        ...
                        // make sure it points out
                        if(edge->vertex == verts[3]) // JRB Dec 30th, was verts[4] - out of bounds
                            edge = edge->mate;
                        ...
                        float alpha = req_length / length(ray);
                        edge->alpha = alpha;
                        edge->alpha_length = req_length;
                    }
                }
    ...
    float v1_alpha, v2_alpha, v3_alpha;
...
void CleaverMesherImp::computeTripleForFace(HalfFace *face)
{
    ...
    */
    ...
    int axis;
    float nx = fabs(n.dot(vec3(1,0,0)));
    float ny = fabs(n.dot(vec3(0,1,0)));
    float nz = fabs(n.dot(vec3(0,0,1)));
...
//===========================================================================================
bool triangle_intersect(Vertex *v1, Vertex *v2, Vertex *v3, vec3 origin, vec3 ray, vec3 &pt, float &error, float EPS)
{
    float epsilon = 1E-7;
