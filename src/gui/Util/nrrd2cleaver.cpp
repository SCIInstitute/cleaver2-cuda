...
AbstractScalarField* loadNRRDFile(const std::string &filename, bool verbose)
{
    ...
    // set scale
    float xs = ((Nrrd*)nin)->axis[0].spacing;
    float ys = ((Nrrd*)nin)->axis[1].spacing;
    float zs = ((Nrrd*)nin)->axis[2].spacing;
...
{
    std::ofstream nrrd_file("charge_density.nrrd");
    if(!nrrd_file.is_open())
    {
        std::cerr << "Problem opeing " << filename << " to write." << std::endl;
    }

    int w = field->dataBounds().size.x;
    int h = field->dataBounds().size.y;
    int d = field->dataBounds().size.z;
    int origin_x = field->bounds().minCorner().x;
    int origin_y = field->bounds().minCorner().y;
    int origin_z = field->bounds().minCorner().z;
